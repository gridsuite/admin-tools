...
from functions.networks.networks import migrate_v214_tapchangersteps
...
parser = argparse.ArgumentParser(description='Send requests to the gridsuite services to migrate V2.14 tap changer steps', )
...
print("---------------------------------------------------------")
print("V2.14 tap changer steps migration script")
if dry_run:
...
print("V2.14 tap changer steps migration (dry-run=" + str(dry_run) + ") in processing...")
failCount = 0
successCount = 0
for network in tqdm(networks):
    ...
    for variant in variants:
        if not dry_run:
            try:
                migrate_v214_tapchangersteps(network['uuid'], variant['num'])
                successCount += 1
...
print("End of V2.14 tap changer steps migration")
