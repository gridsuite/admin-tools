...
from functions.networks.networks import migrate_v221_limits
...
parser = argparse.ArgumentParser(description='Send requests to the gridsuite services to migrate V2.21 limits', )

parser.add_argument("-n", "--dry-run", help="test mode (default) will not execute any request",
                    action="store_true")
...
print("---------------------------------------------------------")
print("V2.21 limits migration script")
if dry_run:
...
print("V2.21 limits migration (dry-run=" + str(dry_run) + ") in processing...")
failCount = 0
successCount = 0
for network in tqdm(networks):
    ...
    for variant in variants:
        if not dry_run:
            try:
                migrate_v221_limits(network['uuid'], variant['num'])
                successCount += 1
...
print("End of V2.21.0 limits migration")
