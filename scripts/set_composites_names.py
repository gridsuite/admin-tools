...
print("Updates the following " + str(len(directory_composites_pairs)) + " composites names : ")
for uuid, name in directory_composites_pairs:
    print(" - ", uuid, '-> ' + name)
    response = requests.put(url = constant.PUT_COMPOSITES.format(uuid = uuid), json={}, params={"name": name})
