...
def delete_orphan_contingency_lists(script_mode):
    ...
    get_directory_contingency_lists_response = requests.get(constant.GET_DIRECTORY_ELEMENTS, params={"elementType": "CONTINGENCY_LIST"})
    get_directory_contingency_lists_response_json = get_directory_contingency_lists_response.json()
    get_directory_contingency_lists_response_json_uuid = map(get_directory_element_uuid, get_directory_contingency_lists_response_json)
